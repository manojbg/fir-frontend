...
const UserDashboard = () => {
  ...
  const handleShow = (firNumber, fileName, type, pk, io, psi, createdDateTime) => {
    ...
    form = form+".html";
    const iframeSrc = `/Forms/`+form+`?firNumber=${encodedFIR}&formName=${encodedForm}&type=`+type+`&pk=`+pk+`&io=`+io+`&psi=`+psi+`&ioa=`+userName+`&cdt=`+createdDateTime+`&closeModal=true`;
     ...
     <tr key={index}>
      ...
      <td>
        <button
          ...
          onClick={() => handleShow(task.FirDTO.FirNumber, document.FileName ,"edit", document.Pk, task.FirDTO.InvestigationOfficer, task.FirDTO.PsiName, document.CreatedDateTime)}
        ></button>
          ...
          <div className="popup-content">
            ...
            >
              ...
              {uploadPopup.UnApprovedFirSupportingDocuments.map((document) => (
                ...
                <label className="required-field"><b>Select File To Upload (pdf only: 10MB)</b></label><br/>
